...
```{r}
salary_aggr_by_industry_count <- salary_aggr_by_industry_count %>% 
  group_by(med_min, med_med, med_max, count) %>% 
  summarise(
    industries_name = paste(sort(industries_name), collapse = ", "),
    .groups = "drop"
  )
```
...
ui <- fluidPage(
  ...
  fluidRow(
    column(12,
           plotlyOutput("plot", height = "300px", width = "100%"))))
  ...
  fluidRow(
    column(12,
           sliderInput("year_range",
                       ...
                       min = as.numeric(format(min(grad_unemp$date), "%Y")),
                       max = as.numeric(format(Sys.Date(), "%Y")),
                       value = c(as.numeric(format(min(grad_unemp$date), "%Y")),
...
server <- function(input, output) {
  ...
  output$linePlot <- renderPlotly({
    ...
    grad_filtered <- grad_unemp %>% filter(date >= start_date & date <= end_date)
    us_filtered   <- us_unemp   %>% filter(date >= start_date & date <= end_date)
    ...
    p <- ggplot(grad_filtered, aes(x = date, y = value)) +
      ...
      labs(title = paste("Data from", input$year_range[1], "to", input$year_range[2]),
